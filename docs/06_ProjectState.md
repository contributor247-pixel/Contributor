...
- **Status:** In progress — Step 2.1 (Auth.js configuration + Drizzle adapter) complete. Steps 0.1-0.4 and 0.6-0.9 (account/env setup) are being handled directly by the user; `DATABASE_URL` and `AUTH_SECRET` are now set in `.env.local` (agent generated AUTH_SECRET via `openssl rand -base64 32` since it needs no external account). Stripe/Resend keys are still placeholders pending the user — not needed until Step 2.3 (Resend) and Step 7 (Stripe).
- **Next up:** Step 2.2 (Signup flow + auth modal — split-panel pattern per docs/02_ThemeGuideline.md Section 5 and Refrence/login.png).
